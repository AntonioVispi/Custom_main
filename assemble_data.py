...
#from scipy import misc   #levato
import imageio  #aggiunto
from PIL import Image  #aggiunto
import os
...
for family in tqdm(file_name_arr):
	...
	for i, file in enumerate(glob.glob(atri_dir+family+'*.png')):
		# Read the image
		read_image = imageio.imread(file, as_grey=True)   #misc.imread(file, flatten=True)
		...
		if i == 0:
			mask = imageio.imread(mask_dir+family+'_segmentation.png', as_grey=True)   #misc.imread(mask_dir+family+'_segmentation.png', flatten=True)
	...
	PIL_image = Image.fromarray(np.uint8(base_image))   #Aggiunto
	PIL_image.save(output_dir+family+'_semantic.png')   #Aggiunto
